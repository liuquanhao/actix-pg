...
impl TodoRepo {
    pub async fn create_todo(&self, create_todo: CreateTodo) -> Result<Todo, TodoRepoError> {
        let todo = Todo {
            id: Uuid::new_v4(),
    ...
    pub async fn delete_todo(&self, id: Uuid) -> Result<u64, TodoRepoError> {
    ...
    pub async fn get_todo(&self, id: Uuid) -> Result<Todo, TodoRepoError> {
        let row = match self.client.query_one(&self.get_todo, &[&id]).await {
            Ok(row) => row,
            Err(_) => return Err(TodoRepoError::NotFound),
        };
    ...
    pub async fn list_todo(&self, pagination: Pagination) -> Result<Vec<Todo>, TodoRepoError> {
        let offset =
